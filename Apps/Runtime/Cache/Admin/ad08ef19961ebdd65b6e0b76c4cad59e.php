...
<html>
  ...
  <div id="main">
         ...
         <div class="news-add">
            ...
            <form action="/blog/index.php/Admin/Article/save" method="post" id="article-form">
			 <script type="text/plain" id="editor" name="content">
               ...
               <input type="text" name="title" id="title" class="box-control" placeholder="填写文章标题" autocomplete="off" value="<?php echo ($arr['atitle']); ?>"/>
               <label for="author">*文章作者/来源</label>
                    <input type="text" name="author" id="author" class="box-control" placeholder="填写文章标题" autocomplete="off" value="<?php echo ($arr['aauthor']); ?>"/>
               <label for="belong">文章所属模块</label>
               <select  class="box-control" id="mid" name="mid">
                   <?php if(is_array($module)): $i = 0; $__LIST__ = $module;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["mid"]); ?>" <?php if($vo['mid'] == $arr['mid']): ?>selected<?php endif; ?> ><?php echo ($arr['Module']['mname']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
               </select>
   ...
   var ue=UE.getEditor('editor');
    ue.ready(function() {
    	   ue.setContent('<?php echo ($arr['acontent']); ?>');
    });
   </script>
 </body>
 </html>